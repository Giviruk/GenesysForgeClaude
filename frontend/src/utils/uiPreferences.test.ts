...
describe('UI preferences persistence', () => {
  ...
  it('ignores corrupt sheet tabs and range tracker values', () => {
    ...
    expect(readRangeTrackerState('c1', 's1')).toEqual({ zones: { valid: 'long' }, log: ['move'] })
  })

  it('stores range tracker state independently for each scene', () => {
    writeRangeTrackerState('c1', 's1', { zones: { p1: 'extreme' }, log: ['move'] })

    expect(readRangeTrackerState('c1', 's1')).toEqual({ zones: { p1: 'extreme' }, log: ['move'] })
    expect(readRangeTrackerState('c1', 's2')).toEqual({ zones: {}, log: [] })
  })
})
