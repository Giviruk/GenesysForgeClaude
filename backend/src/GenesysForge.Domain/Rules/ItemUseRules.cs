...
public static class ItemUseRules
{
    ...
    /// что несут в бою: у оружия отваливается урон, у брони — поглощение. Контейнер книга называет
    /// отдельно — серьёзно повреждённый теряет прибавку к порогу веса, но не содержимое. Мотку
    /// верёвки и рациону ломаться нечем, и «порог поломки» у них был просто шумом на карточке.
    /// </summary>
    public static bool CanBeDamaged(ItemDef def) =>
        def.Kind is ItemKind.Weapon or ItemKind.Armor || def.EncumbranceThresholdBonus != 0;
}
