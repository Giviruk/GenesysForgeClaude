...
public static class SeedData
{
    ...
    private static IEnumerable<SpellDef> Spells(GameSystem sys)
    {
        ...
        {
            ("Attack", "Атака", "1 (Easy)",
                "Боевое магическое действие против одной цели на короткой дистанции: наносит урон, равный характеристике используемого магического навыка, +1 за каждый неотменённый успех. По умолчанию не имеет критического значения.",
            ...
            ("Barrier", "Барьер", "1 (Easy)",
                "Создаёт магическую защиту для цели вплотную с заклинателем; до конца следующего хода уменьшает урон от попаданий по цели.",
            ...
            ("Dispel", "Рассеивание", "3 (Hard)",
                ...
                ["Arcana", "Verse"], null, 6),
            ("Heal", "Лечение", "1 (Easy)",
                "Магическое лечение ран и усталости у цели вплотную; успехи лечат раны, преимущества лечат усталость.",
        ...
        var additional = new (string Parent, string Ru, string En, string Diff, string Desc, string Safe, string? SrcOverride, int Sort)[]
        {
            ...
            ("Attack", "Взрывной", "Blast", "+1",
                "Атака получает свойство «Взрыв» с рейтингом, равным рангу Знания заклинателя.",
                "Добавляет свойство «Взрыв».", null, 2),
            ("Attack", "Двигающий", "Move", "+1",
                "Если атака попадает, можно потратить преимущество, чтобы переместить цель на один диапазон дистанции в любом направлении. Только Магия/Arcana.",
                "При попадании перемещает цель (только Arcana).", null, 3),
            ...
            ("Attack", "Ледяной", "Ice", "+1",
                "Атака получает свойство «Сковывание» с рейтингом, равным рангу Знания заклинателя.",
                "Добавляет «Сковывание».", null, 5),
            ("Attack", "Молниеносный", "Lightning", "+1",
                "Атака получает «Оглушение» с рейтингом, равным рангу Знания, а также свойство «Автоматическое». При использовании Автоматического свойства сложность увеличивается дополнительно по обычным правилам.",
                "Добавляет «Оглушение» и «Автоматическое».", null, 6),
            ("Attack", "Нелетальный", "Non-Lethal", "+1",
                "Атака получает свойство «Оглушающий урон». Только Природа/Primal.",
                "Добавляет «Оглушающий урон» (только Primal).", null, 7),
            ("Attack", "Огненный", "Fire", "+1",
                "Атака получает свойство «Жжение» с рейтингом, равным рангу Знания заклинателя.",
                "Добавляет «Жжение».", null, 8),
            ...
            ("Attack", "Смертельный", "Deadly", "+1",
                "Атака получает критическое значение 2 и свойство «Высококритичное» с рейтингом, равным рангу Знания.",
                "Добавляет критическое значение 2 и «Высококритичное».", null, 10),
            ("Attack", "Ударный", "Impact", "+1",
                "Атака получает «Нокдаун» и «Дезориентацию» с рейтингом, равным рангу Знания.",
                "Добавляет «Нокдаун» и «Дезориентацию».", null, 11),
            ("Attack", "Управляющий", "Manipulative", "+1",
                "Если атака попадает, можно потратить преимущество, чтобы переместить цель на один диапазон дистанции в любом направлении. Только Магия/Arcana.",
                "При попадании перемещает цель (только Arcana).", null, 12),
            ("Attack", "Разрушительный", "Destructive", "+2",
                "Атака получает свойство «Повреждение» и «Проникающее» с рейтингом, равным рангу Знания.",
                "Добавляет «Повреждение» и «Проникающее».", null, 13),
            ...
            ("Attack", "Ядовитый", "Poisonous", "+2",
                "Если атака наносит урон, цель немедленно совершает сложную проверку Стойкости; при провале получает раны и усталость, равные рангу Знания. Считается действием яда.",
                "При попадании — проверка Стойкости или раны/усталость.", null, 15),
            // Barrier
            ...
            ("Barrier", "Дополнительная цель", "Additional Target", "+1",
                "Заклинание воздействует на одну дополнительную цель в пределах дистанции; после наложения можно тратить преимущества на новые цели.",
                "Добавляет ещё одну цель.", null, 2),
            ...
            ("Barrier", "Отражающий", "Reflective", "+2",
                "Если противник магически атакует защищённую цель и получает достаточно угрозы или отчаяния, он сам получает попадание с уроном, равным итоговому урону своей атаки. Только Магия/Arcana.",
                "Отражает магические атаки обратно (только Arcana).", null, 4),
            ...
            ("Heal", "Дополнительная цель", "Additional Target", "+1",
                "Заклинание воздействует на одну дополнительную цель в пределах дистанции; после наложения можно тратить преимущества на новые цели.",
                "Добавляет ещё одну цель.", null, 3),
            ...
            ("Conjure", "Дополнительный призыв", "Additional Summon", "+1",
                "Создаёт один дополнительный предмет, оружие или существо; после наложения можно тратить преимущества на новые дополнительные призывы.",
                "Призывает ещё один предмет или существо.", null, 2),
            ...
            ("Curse", "Дополнительная цель", "Additional Target", "+2",
                "Заклинание воздействует на одну дополнительную цель в пределах дистанции; после наложения можно тратить преимущества на новые цели.",
                "Добавляет ещё одну цель.", null, 4),
            ...
            ("Augment", "Дополнительная цель", "Additional Target", "+2",
                "Заклинание воздействует на одну дополнительную цель в пределах дистанции; после наложения можно тратить преимущества на новые цели.",
                "Добавляет ещё одну цель.", null, 6),
            // Mask (EPG)
            ...
            ("Mask", "Зеркальные образы", "Mirror Image", "+1",
                "Создаёт несколько иллюзорных копий, которые отвлекают противников; часть атак может попасть в копию вместо цели.",
                "Иллюзорные копии отвлекают атаки.", epgSource, 2),
            ("Mask", "Дополнительная иллюзия", "Additional Illusion", "+1",
                "Создаёт ещё одну иллюзию или маскирует ещё одного персонажа; после наложения можно тратить преимущества на новые иллюзии.",
                "Добавляет иллюзию или маскировку.", epgSource, 3),
            ...
            ("Mask", "Реализм", "Realism", "+1",
                "Увеличивает сложность проверок, чтобы распознать иллюзию; также позволяет обманывать дополнительные чувства вроде запаха, вкуса или осязания.",
                "Иллюзию труднее распознать; обманывает больше чувств.", epgSource, 6),
            ("Mask", "Ужас", "Terror", "+2",
                "Иллюзия пугает тех, кто не знает, что она ненастоящая; наблюдатели проходят сложную проверку Дисциплины, иначе не могут приблизиться.",
                "Иллюзия пугает тех, кто не знает правды.", epgSource, 7),
            ...
            ("Predict", "Дополнительные вопросы", "Additional Questions", "+1",
                "Позволяет задать один дополнительный вопрос; после наложения можно тратить преимущества на новые дополнительные вопросы.",
                "Добавляет ещё один вопрос.", epgSource, 4),
            ("Predict", "Вспышка предвидения", "Flash of Precognition", "+2",
                "Помимо вопроса, даёт один будущий бонус к собственной проверке и один защитный бонус против проверки, нацеленной на персонажа. После наложения можно потратить преимущества, чтобы усилить эти бонусы.",
                "Бонус к своей проверке и защита от чужой.", epgSource, 5),
            ("Predict", "Обмануть смерть", "Cheat Death", "+2",
                "Помимо вопроса, персонаж предвидит возможную гибель; до конца сессии можно потратить очко сюжета, чтобы избежать вывода из строя или смерти. Персонаж получает раны и усталость до порогов, но не превышает их.",
                "Тратя очко сюжета, избегает гибели.", epgSource, 6),
            // Transform (EPG)
