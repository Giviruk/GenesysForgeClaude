    ...
    private static IEnumerable<SpellDef> Spells(GameSystem sys)
    {
        ...
        var effects = new (string En, string Ru, string Diff, string Desc, string Safe, string SafeEn, string? SrcOverride, int Sort)[]
        {
            ...
            ("Augment", "Усиление", "2 (Average)",
                "Временно усиливает цель вплотную с заклинателем; базово повышает одну характеристику для всех проверок навыков. На цели действует только одно усиление одновременно: пока прежнее не закончилось, новое наложение на эту цель отклоняется до броска, а если усилена хотя бы одна из выбранных целей, отклоняется всё заклинание. Эффекты двух заклинаний не складываются.",
                "Временно усиливает характеристику цели; на цели действует только одно усиление.",
                "Temporarily boosts a characteristic of the target; only one augment affects a target at a time.",
                null, 2),
            ...
            ("Conjure", "Призыв", "1 (Easy)",
                "Создаёт простой предмет без движущихся частей, одноручное оружие ближнего боя без движущихся частей или временного приспешника силуэта не больше 1. Созданное появляется вплотную к заклинателю и существует до конца его следующего хода; манёвр концентрации в последующие ходы продлевает существование, а без концентрации призванное исчезает. Призванное существо действует по своим инстинктам, заклинателю не подчиняется и может оказаться враждебным; в структурированном столкновении оно ходит сразу после заклинателя.",
                "Создаёт предмет или приспешника; призванное существо заклинателю не подчиняется.",
                "Creates an item or a minion; a summoned creature does not obey the caster.",
                null, 4),
        ...
        var additional = new (string Parent, string Ru, string En, string Diff, string Desc, string Safe, string SafeEn, string? SrcOverride, int Sort)[]
        {
            ...
            ("Conjure", "Дополнительный призыв", "Additional Summon", "+1",
                "Заклинание призывает один дополнительный предмет, оружие или существо. После наложения можно потратить 2 преимущества, чтобы призвать ещё один дополнительный предмет, оружие или существо; это можно повторять, каждый раз тратя 2 преимущества. Одного преимущества недостаточно. Все дополнительные призывы подчиняются выбранному уровню призыва и его ограничениям.",
                "Добавляет призыв; после броска +1 призыв за 2 преимущества.",
                "Adds a summon; after the roll, +1 summon per 2 advantages.", null, 2),
            ...
            ("Augment", "Быстрота", "Swift", "+1",
                "Пока действует заклинание, цели игнорируют эффекты пересечённой местности и не могут быть обездвижены.",
