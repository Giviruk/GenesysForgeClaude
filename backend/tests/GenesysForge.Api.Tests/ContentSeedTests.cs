...
public class ContentSeedTests
{
    ...
    [Fact]
    public void PrivateFull_Reseed_UpdatesExistingTalentDescriptions()
    {
        using var db = NewDb();
        ...
        {
            ["rot.talent.povyshennoe-vnimanie"] =
            ...
            ["rot.talent.ostroumnyy-otvet"] =
                "Один раз за сцену персонаж может использовать этот талант, чтобы автоматически добавить ▼ ▼ к проверке социального навыка другого персонажа.",
            ["rot.talent.bystryy-udar"] =
                "Персонаж добавляет ◻ за каждый ранг таланта ко всем боевым проверкам против всех целей, ещё не совершавших ходов в этой сцене.",
            ["rot.talent.duelyant"] =
                "Во время проверок ближнего боя только с одним противником персонаж добавляет ◻ ко всем проверкам ближнего боя. Если же персонаж находится в ближнем бою с тремя и более противниками, то он добавляет ■ ко всем проверкам ближнего боя.",
            ["rot.talent.parirovanie-u-luchshennyy"] =
                "Прежде вы должны приобрести талант Парирование. Когда персонаж получает попадание атаки ближнего боя и использует Парирование для снижения урона от этого попадания, то после осуществления этой атаки персонаж может потратить ☠, или ▼ ▼ ▼ из результатов проверки противника для использования этого таланта. Тогда персонаж автоматически попадает по атакующему Грубой силой или оружием, используемым для навыка Ближний бой. Попадание наносит базовый урон оружия, а также любой урон от применимых в этой ситуации талантов или способностей. Персонаж не может использовать этот талант, если исходная атака вывела его из строя.",
            ["rot.talent.izobretatel"] =
                "Создавая новые предметы или модифицируя имеющиеся, персонаж может использовать этот талант, чтобы добавить к проверке ◻ за каждый ранг таланта. Кроме того, персонаж может попытаться воссоздать устройства, про которые он слышал, но не видел и не имеет никаких связанных с ними планов или схем.",
        };

        // Имитируем засиженную приватную БД с прежними OCR-описаниями.
        foreach (var talent in db.TalentDefs.Where(t => expected.ContainsKey(t.Code)))
            talent.Description = "старое OCR-описание О []";
        db.SaveChanges();

        SeedData.Apply(db, ContentMode.PrivateFull);

        foreach (var (code, description) in expected)
            Assert.Equal(description, db.TalentDefs.Single(t => t.Code == code).Description);
    }
