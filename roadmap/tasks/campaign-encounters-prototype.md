...
- [x] PR открыт
- [x] Подогнать глобальные пропорции shell/tabs/panels под HTML-прототипы docs, чтобы вкладка энкаунтеров и остальной сайт визуально совпадали по плотности
- [x] Перенести название, тип, сложность и запуск сцены в верхнюю строку энкаунтера, убрать отдельный блок `Запуск`
...
- Дополнительная подгонка: `shell` расширен до ширины прототипов, вкладки и панели сделаны плотнее, верхний заголовок приведен к формату `раздел + рабочая область`.
- Уточнение layout: отдельный блок `Запуск` убран; у мастера запуск находится в шапке сцены рядом с печатью, а основные параметры сцены редактируются там же.
